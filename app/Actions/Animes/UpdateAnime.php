<?php

namespace AnimeSite\Actions\Animes;

use Illuminate\Http\UploadedFile;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Gate;
use AnimeSite\Models\Anime;

class UpdateAnime
{
    /**
     * @param Anime $anime
     * @param array{
     *     slug?: string,
     *     name?: string,
     *     description?: string,
     *     image_name?: string|UploadedFile,
     *     studio_id?: string,
     *     countries?: array,
     *     poster?: string|UploadedFile|null,
     *     imdb_score?: float|null,
     *     first_air_date?: string|null,
     *     last_air_date?: string|null,
     *     attachments?: array,
     *     related?: array|null,
     *     similars?: array,
     *     is_published?: bool,
     *     meta_title?: string|null,
     *     meta_description?: string|null,
     *     meta_image?: string|UploadedFile|null,
     * } $data
     */
    public function __invoke(Anime $anime, array $data): Anime
    {
        Gate::authorize('update', $anime);

        return DB::transaction(function () use ($anime, $data) {
            // Handle file uploads
            if (isset($data['image_name']) && $data['image_name'] instanceof UploadedFile) {
                $data['image_name'] = $anime->handleFileUpload($data['image_name'], 'images', $anime->image_name);
            }

            if (isset($data['poster']) && $data['poster'] instanceof UploadedFile) {
                $data['poster'] = $anime->handleFileUpload($data['poster'], 'posters', $anime->poster);
            }

            if (isset($data['meta_image']) && $data['meta_image'] instanceof UploadedFile) {
                $data['meta_image'] = $anime->handleFileUpload($data['meta_image'], 'meta', $anime->meta_image);
            }

            // Process attachments if they exist
            if (isset($data['attachments']) && is_array($data['attachments'])) {
                $data['attachments'] = $anime->processAttachments($data['attachments'], 'attachments');
            }

            // Update the model with all data
            $anime->update($data);

            return $anime;
        });
    }
}
